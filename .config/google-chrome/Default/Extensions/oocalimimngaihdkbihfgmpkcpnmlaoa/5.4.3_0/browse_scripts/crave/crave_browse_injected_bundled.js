...
function addNativePartyButton() {
    ...
    const buttons = document.querySelectorAll(".bg-button-primary");
    const button = Array.from(buttons).find((btn) => btn.tagName === "A" && btn.getAttribute("href") && btn.getAttribute("href").includes("/play"));
    if (button == null) {
        return undefined;
    }
