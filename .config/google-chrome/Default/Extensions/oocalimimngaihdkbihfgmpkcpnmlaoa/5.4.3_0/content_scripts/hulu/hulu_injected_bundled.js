...
const getOffset = function () {
    ...
    const currentTime = videoEl === null || videoEl === void 0 ? void 0 : videoEl.currentTime;
    let offset = 0;
    if (playerEl) {
...
const getCurrentTime = function () {
    ...
    if (IS_SAFARI) {
        ...
        const updatedTime = Math.round((videoEl === null || videoEl === void 0 ? void 0 : videoEl.currentTime) - offset);
        ...
        return Math.round(videoEl === null || videoEl === void 0 ? void 0 : videoEl.currentTime);
    }
};
var seekInteraction = function (event) {
    ...
    switch (event_type) {
        case "SEEK": {
            if (getHuluPlayer()) {
                getHuluPlayer().seek(Math.round(event.data.time / 1000));
            }
            else {
                document.querySelector("#content-video-player").__HuluDashPlayer__.currentTime = Math.round(event.data.time / 1000);
            }
            break;
        }
        case "UpdateState": {
            const paused = getHuluPlayer().paused;
            const currentTime = getCurrentTime() * 1000;
            const evt = new CustomEvent("FromNode", {
                detail: { type: "UpdateState", paused, currentTime, updatedAt: Date.now() },
            });
        ...
        case "CHECK_FORCE_AD": {
            if (checkForceAd()) {
                const evt = new CustomEvent("FromInjected", {
                    detail: { type: "CHECK_FORCE_AD", forceAd: true, updatedAt: Date.now() },
                });
                window.dispatchEvent(evt);
            }
            break;
        ...
        case "ON_AD_START": {
            ...
            if (player && adBreaks.length > 0) {
                window.lastPlayedTime = document.querySelector("video").currentTime;
            }
            break;
        }
        case "ON_AD_END": {
            ...
            if (player && window.lastPlayedTime > 0) {
                // player.seek(window.lastPlayedTime)
                window.lastPlayedTime = 0;
            }
            break;
        }
