...
async function requestMicrophonePermission() {
  ...
  try {
    ...
    hideLoading();
    showStatus('✅ Microphone access granted! You\'re all set.', 'status-success');
    ...
    if (error.name === 'NotAllowedError') {
      if (error.toString().includes('dismissed')) {
        // Allowed to retry
        errorMessage = 'Microphone access was denied.<br/>Tap "Allow while visiting the site" to grant the access.';
      } else {
        // User clicked on "Never allow" - so they need to now manually re-enable it
        errorMessage = 'Microphone access not available. Please visit <a class="permission-link" href="#">this link</a> and enable the Microphone permission.';
      }
